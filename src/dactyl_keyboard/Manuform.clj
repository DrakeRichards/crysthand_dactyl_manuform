...
(defn lower2thumbRoyY [mod] 
  + mod -33)
(defn boo [shape] (->>  shape 
  ; (rotate (deg2rad -7) [1 0 0])
  (translate (map - thumborigin))
  (rotate (deg2rad 12) [1 -1 0])
  (rotate (deg2rad -4) [0 0 1])  
...
(def original_usb_holder_position (key-position 0 0 (map + (wall-locate2 0 1) [0 (/ mount-height 2) 0])))
...
(defn rounded-cube [x y z r]
    (let [
        corner (cylinder [r r] z)
        hx (/ x 2.0)
        hy (/ y 2.0)
        hxr (- hx r)
        hyr (- hy r)        
        corners (union 
            (translate [hxr hyr 0] corner)
            (translate [(- hxr) hyr 0] corner)
            (translate [(- hxr) (- hyr) 0] corner)
            (translate [hxr (- hyr) 0] corner)
          )
        rr (* r 2.0)
        cube_cut (cube rr rr (* z 1.1))
        corners_cut (union 
            (translate [hx hy 0] cube_cut)
            (translate [(- hx) hy 0] cube_cut)
            (translate [(- hx) (- hy) 0] cube_cut)
            (translate [hx (- hy) 0] cube_cut)
          )
...
(defn usbc_cutout [thickness]
  (let [
    hole (with-fn 20 (cylinder [1.6 1.6] thickness))
    ]
    (union 
...
(def usbc-holderr
    (difference 
      (rounded-cube 23.5 8.8 3.0 1.0)            
      (usbc_cutout wall-thickness)
      )
      )
(def usbc-holder usbc-holderr)



(defn dimo [x] (+ x (* 2 wall-thickness)))

(def panel_x 50.0)
(def panel_y wall-thickness)
(def panel_z 14.0)
; (def back-panel-origin [-10 69 14])
...
(def back-panel-origin    
  [(- (first back-panel-ref) 12) (second back-panel-ref) 10]
)
...
(def back-panel-hole  
  (let [
    ...
    (translate back-panel-origin 
      (cube panel_x (+ clearance panel_y) panel_z)      
    )    
  )
)
(def outshot 20)
(def back-panel-hole-inset  
    (translate back-panel-origin 
      (translate [0 (/ outshot 2) 0]  
        (cube panel_x (+ outshot panel_y) panel_z)
      )
    )    
  )  
  ...
  (let [
      ...
      back-plate 
        (minkowski 
          ...
          (cylinder panel_y half_wall)
        )      
    ]            
    (union
      (translate back-panel-origin 
        (translate [0 (- (/ (+ panel_y wall-thickness) 2 )) 0]
          (difference 
            (translate [0 0 0] 
              (rotate (/ (- pi) 2) [1 0 0]                 
                back-plate
            ...
            (let [
              ...
              (union 
                (rotate (/ (- pi) 2) [1 0 0] 
                  (translate [-5 0 0] 
                    (translate [10 0 0] 
                        (usbc_cutout thickness-cut)
                    )
                    (translate [-14 0 0] 
                        (union 
                          (with-fn 20 (cylinder [din_hole_radius din_hole_radius] thickness-cut))
                          (translate [0 0 (/ (- wall-thickness din_hole_outset_depth) -2)]
                            (with-fn 20 (cylinder [din_hole_outset_radius din_hole_outset_radius] din_hole_outset_depth))
                          )
                        )
                    )
                  )
...
(def the_thing_that_should_cube_outlier3 
  (translate back-panel-origin 
        (let [
          back_of_panel (- (/ panel_y 2))
          half_wall 0.5 ; (- (/ wall-thickness 2))
          pos (+ back_of_panel half_wall)
          thickness-cut 20
          ]
          (translate [0 pos 0] (usbc_cutout thickness-cut))
        )
      )    
)
...
(def model-right 
  (union 
    (difference
      (union
        key-holes
        ; the_thing_that_should_cube_outlier3 
        ; (translate [100 100 100] case-wall-cutout)
        ; (translate [100 100 100] back-panel)
        back-panel
        connectors
        thumb
        thumb-connectors            
